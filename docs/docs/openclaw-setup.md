...
登录完成后即可对话发布文章，示例见 [使用案例 - 案例三：文章发布到掘金](use-cases.md#case-3)。
...
安装完成后测试基础转码（生成测试视频并转 GIF），验证命令示例见 [使用案例 - 案例四：音视频处理](use-cases.md#case-4)。

没有报错说明 ffmpeg 安装正常。

### 日常使用

安装完成后即可在对话中直接让 Agent 处理音视频（转格式、转 GIF、提取音频等），使用示例见 [使用案例 - 案例四：音视频处理](use-cases.md#case-4)。
