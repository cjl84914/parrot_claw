...
typedef OpenClawExtensionDirectoriesLoader =
    Future<List<Directory>> Function();
typedef OpenClawPathJoiner = String Function(String parent, String child);

/// Shared OpenClaw model configuration implementation for desktop platforms.
///
/// Platform-specific configuration paths and extension discovery are injected
/// by the concrete platform wrappers.
class OpenClawModelServiceImpl implements OpenClawModelService {
  OpenClawModelServiceImpl({
    required String configFilePath,
    required OpenClawExtensionDirectoriesLoader
        extensionDirectoriesLoader,
    required OpenClawPathJoiner pathJoiner,
  }) : _configFile = File(configFilePath),
       _extensionDirectoriesLoader = extensionDirectoriesLoader,
       _pathJoiner = pathJoiner;

  final File _configFile;
  final OpenClawExtensionDirectoriesLoader _extensionDirectoriesLoader;
  final OpenClawPathJoiner _pathJoiner;
  ...
  Future<Map<String, dynamic>> _readConfig() async {
    if (!await _configFile.exists()) return <String, dynamic>{};
  ...
  Future<Map<String, Set<String>>> _readInstalledPluginProviderUrls() async {
    final result = <String, Set<String>>{};
    for (final directory in await _extensionDirectoriesLoader()) {
      final metadataFile = File(_pathJoiner(directory.path, 'openclaw.plugin.json'));
