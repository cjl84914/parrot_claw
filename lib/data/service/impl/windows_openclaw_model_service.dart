...
class WindowsOpenClawModelService implements OpenClawModelService {
  @override
  File get configFile => File(WindowsOpenClawEnvironment.configFilePath);
  ...
  Future<List<Directory>> _openClawExtensionDirectories() async {
    final roots = <Directory>[
      // Extensions installed for the current OpenClaw profile.
      Directory('${WindowsOpenClawEnvironment.openClawHome}\\extensions'),
    ];

    // npm resolves the global package root itself. Do not infer it from
    // APPDATA: npm prefix/root can be overridden by the user's configuration.
  ...
  Future<String?> _readNpmGlobalRoot() async {
    try {
  ...
  String _firstNonEmptyLine(dynamic value) {
