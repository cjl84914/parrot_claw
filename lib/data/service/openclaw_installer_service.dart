...
class OpenClawInstallerService {
  OpenClawInstallerService({Logger? logger})
      : _log = logger ?? Logger('OpenClawInstallerService');

  final Logger _log;

  /// Node 二进制镜像根（npmmirror 淘宝源）
  static const String nodeMirrorBase =
      'https://npmmirror.com/mirrors/node';

  /// npm registry 镜像
  static const String npmRegistryMirror =
  ...
  /// 本地安装的 Node 目录（完整路径）
  String get nodeHomePath => '${LocalEnvConfig.homePath}/$nodeInstallDir';
  ...
  Future<int> installOpenClaw({
    ...
    try {
      ...
      final process = await Process.start(
        ...
        environment: {
          ...LocalEnvConfig.processEnvironment,
          // 确保 npm 使用 node 同目录
          'PATH': LocalEnvConfig.pathWithPrepended(nodeDir),
        },
        runInShell: true,
      );
