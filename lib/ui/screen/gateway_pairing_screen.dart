...
class GatewayPairingScreen extends StatelessWidget {
  ...
  const GatewayPairingScreen({required this.requestId, super.key});

  @override
  Widget build(BuildContext context) {
    final command = 'openclaw devices approve $requestId';
    return Scaffold(
                  ...
                  : () async {
                    ...
                    if (context.mounted)
                      ScaffoldMessenger.of(
                        context,
                      ).showSnackBar(const SnackBar(content: Text('授权命令已复制')));
                  },
  ...
  Widget build(BuildContext context) => Row(
    children: [
      Container(
        ...
        height: 12,
        decoration: BoxDecoration(
          color: active ? AppColors.secondary : const Color(0xFF666666),
          shape: BoxShape.circle,
        ),
      ),
